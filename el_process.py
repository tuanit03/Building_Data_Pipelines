...
# Tạo index để tối ưu hóa query cho bước transform
collection.create_index([('category', 1), ('year', 1), ('month', 1)])
...
def get_article_links(cateid, from_date, to_date, page):
    try:
        ...
        time.sleep(random.uniform(0, 16))  # Thêm khoảng nghỉ để tránh bị chặn
...
def scrape_article(link, category_name, year, month):
    logger = logging.getLogger('airflow.task')
    data = {}
    try:
        start_time = time.time()
        ...
        # Không cần cắt chuỗi date_span, vì year và month đã được truyền vào
        data['year'] = year
        data['month'] = month
...
def etl_category(cateid, category_name, year, month, **context):
    ...
    for page in range(1, 21):  # Duyệt qua 20 trang
        ...
        for link in article_links:
            article = scrape_article(link, category_name, year, month)  # Truyền year và month trực tiếp
            if article:
                start_time = time.time()
...
with DAG(
        ...
        description='Extract, Load articles from vnexpress by category, year, and month',
